...
arr_size.addEventListener("input",function(){
    createBarArray(parseInt(no_of_bars));
});

createBarArray(no_of_bars);
...
function createBarArray(no_of_bars = 60) {
    ...
    for (singleBar = 0; singleBar < no_of_bars; singleBar++) {
        random_array_100.push(Math.round(Math.random() * 250) + 1);
    }
